...
- **styled-components**(メンテナンスモード宣言済み)→ **CSS Modules に決定 (2026-07-19, branch: css-migration-trial)** — **全面移行完了 (2026-07-19)**
  ...
  - **実施結果**: `src/theme.css` に `theme.ts` の値を `:root` の CSS カスタムプロパティ
    ...
    popup.tsx / options.tsx の両エントリで import。
    ...
    `src/styled.d.ts` を削除。`options/CodeEditor.tsx` は CSS Modules の対象外となる
    エディタライブラリのテーマ設定で `Theme.ts` を利用する箇所として温存。
...
- **prismjs + react-simple-code-editor** → **`@uiw/react-codemirror` + `@codemirror/lang-javascript` に決定・移行完了 (2026-07-20)**
  - `basicSetup` の自動補完、JavaScript の構文ハイライト、スニペット、ローカル変数補完を有効化
  - 関数引数の `Page` / `Modifier` と sandbox の `render` に対するアプリ固有の補完を追加
  - 既存の CodeEditor の controlled value / sandbox 構文検証 / 編集画面の見た目を維持
  - options E2E に「コードを編集 → 保存 → storage 永続化 → reload 後の表示」を追加
  - VS Code のような外部 API・型情報ベースの IntelliSense は対象外。必要になった時点で TypeScript/LSP 導入を検討
...
- **storage の堅牢化(「関数が多くなると壊れる」の本丸)**:
  - 設計: [docs/function-storage.md](./docs/function-storage.md)
  - `chrome.storage.sync` は 1 item 8KB / 全体 100KB の quota があり、関数が増えるとここで壊れる
  - Pointer / sharded Catalog / 関数ごとのDocumentに分割し、全体100KBまで利用する
  - quota超過時は暗黙にlocalへfallbackせず、保存を拒否してexportまたは削除を案内する
  - Chrome / Firefoxは`storage.sync`、同期非対応のSafariは同じ論理形式を`storage.local`に保存する
